...
import "./IPermanentLocksPoolV2.sol";

interface IERC20Metadata {
  function decimals() external view returns (uint8);
}

/// @title Autopilot Permanent Locks Vault V2 (Batched Individual Voting)
...
contract PermanentLocksPoolV2 is IPermanentLocksPoolV2 {
  ...
  uint256 internal window_preepoch_duration;

  uint256 internal window_postepoch_duration;
  ...
  uint256 internal constant SCALE = 10 ** DECIMALS;
  ...
  /// @notice Timestamp when last snapshot ID was updated
  uint256 internal last_snapshot_id_updated_at;
  ...
  mapping(uint256 => uint256) internal acc_reward_scaled_per_epoch;
  ...
  /// @notice Mapping from lock ID to its owner
  ...
  modifier onlyPermittedOperator() {
    require(permitted_operators[msg.sender], "H");
    _;
  }

  /// @notice Restricts access to swapper contract only
  modifier onlySwapper() {
    require(swapper_contract_address == msg.sender, "I");
    _;
  }
  ...
  constructor(
    ...
    require(_epochs_offset_timestamp > 0, "K");
    ...
    _setLastSnapshotId(_getCurrentEpochId());

    reward_token_decimals = IERC20Metadata(address(_rewards_token)).decimals();
    require(reward_token_decimals <= DECIMALS, "L");
  }
  ...
  function voteWithNfts(
    ...
    uint256 current_epoch_id = _getCurrentEpochId();
    require(current_epoch_id == last_snapshot_id, "R");

    require(_nft_ids.length > 0, "S");
  ...
  function claimVotingRewards(
    ...
    require(_bribes_and_fees.length == _tokens.length, "T");
    require(_nft_ids.length > 0, "V");

    // Claim rewards for each NFT
    for (uint256 i = 0; i < _nft_ids.length; i++) {
      uint256 nft_id = _nft_ids[i];
      require(nft_locks_contract.ownerOf(nft_id) == address(this), "W");
  ...
  /// @notice Resets voting for multiple NFTs
  /// @dev Check which nft was voted before must be done offchain to save gas
  /// @param _nft_ids Array of NFT IDs to reset voting for
  function resetVoting(
    uint256[] calldata _nft_ids
  ) external onlyPermittedOperator {
    for (uint256 i = 0; i < _nft_ids.length; i++) {
      voter_contract.reset(_nft_ids[i]);
    }
  }
  ...
  function claimRebaseRewards(
    ...
    for (uint256 i = 0; i < _nft_ids.length; i++) {
      ...
      require(nft_owner != address(0), "AA");
    ...
    if (total_after_balance != total_before_balance) {
  ...
  function fillSwapper(
    ...
    require(swapper_contract_address != address(0), "AB");
  ...
  function snapshotRewards(uint256 reward_amount) external onlySwapper {
    ...
    _setLastSnapshotId(current_epoch);
  }
  ...
  function setSwapperContract(address _swapper_contract) external onlyOwner {
    require(_swapper_contract != address(0), "3");
  ...
  function transferOwnership(address _new_owner) external onlyOwner {
    require(_new_owner != address(0), "5");
  ...
  function emergencyWithdrawFromRewardsVault(
    ...
    _emergencySnapshot();
    _isNotInSpecialWindowOrFail(last_snapshot_id);
    require(_recipient != address(0), "6");
  ...
  function emergencyWithdrawFromLocksVault(
    ...
    _emergencySnapshot();
    _isNotInSpecialWindowOrFail(last_snapshot_id);
    require(_recipient != address(0), "7");
    require(_amount > 0, "8");
  ...
  /// @return wrapped_start Timestamp when wrapped epoch starts (special window ends)
  /// @return wrapped_end Timestamp when wrapped epoch ends (special window starts)
  /// @return window_start Timestamp when special window starts
  /// @return window_end Timestamp when special window ends
  function getEpochInfo(uint256 _epoch_id) external view returns (
    uint256 epoch_start,
    uint256 epoch_end,
    uint256 wrapped_start,
    uint256 wrapped_end,
    uint256 window_start,
    uint256 window_end,
    uint256 preepoch_duration,
    uint256 postepoch_duration
  ) {
    epoch_start = _getEpochStartAt(_epoch_id); // Thursday 00:00 UTC
    epoch_end = _getEpochEndAt(_epoch_id); // Wednesday 23:59 UTC

    wrapped_start = _getWrappedEpochStartAt(_epoch_id); // Thursday 02:00 UTC
    wrapped_end = _getWrappedEpochEndAt(_epoch_id); // Wednesday 22:00 UTC

    window_start = _getWrappedEpochEndAt(_epoch_id); // Wednesday 22:00 UTC
    window_end = _getWrappedEpochStartAt(_epoch_id + 1); // Thursday 02:00 UTC (next epoch)

    preepoch_duration = window_preepoch_duration;
    postepoch_duration = window_postepoch_duration;
  }
  ...
  /// @dev Special window is when Autopilot bots perform critical operations
  ...
  /// @notice Internal setter for last snapshot ID with timestamp update
  /// @param snapshot_id The new snapshot ID to set
  function _setLastSnapshotId(
		uint256 snapshot_id
	) internal {
		last_snapshot_id = snapshot_id;
    last_snapshot_id_updated_at = block.timestamp;
	}
  ...
  ///      Special window is when Autopilot bots perform critical operations like:
  ///      - Voting with individual NFTs
  ...
  function _setWindowDurations(
    ...
    require(_window_preepoch_duration >= 1.5 hours, "D");
    require(_window_postepoch_duration >= 0.5 hours, "E");
    require(
